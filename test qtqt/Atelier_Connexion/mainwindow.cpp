...
void MainWindow::on_ajouter_clicked()
{
    ...
    if (!nameRx.exactMatch(nom) || !nameRx.exactMatch(prenom)) {
    ...
    } else if (!telRx.exactMatch(telStr) || !telRx.exactMatch(ageStr)) {
        QMessageBox::critical(this, tr("Erreur"), tr("Le téléphone et l'âge doivent être des nombres valides."), QMessageBox::Cancel);
        ...
        client client(0, nom, prenom, tel, adresse, gender, age, email);
...
void MainWindow::on_tableView_activated(const QModelIndex &index)
{
    ...
    qry.prepare("SELECT NOM_CL, PRENOM_CL, GENDER, TEL_CL, ADRESSE_CL, AGE, EMAIL FROM client WHERE CIN = :CIN");
    qry.bindValue(":CIN", value);
...
void MainWindow::on_modifier_clicked()
{
    ...
    int cin = selectedIndexes.at(0).sibling(selectedIndexes.at(0).row(), 0).data().toInt();
    ...
    client client(cin, nom, prenom, tel, adresse, gender, age, email);
...
void MainWindow::on_sortComboBox_currentIndexChanged(int index)
{
    ...
    switch (index) {
        case 1: // Sort by Prénom (A-Z)
            queryStr = "SELECT CIN, NOM_CL, PRENOM_CL, TEL_CL, ADRESSE_CL, GENDER, AGE, EMAIL, 'Update' AS Action, 'Delete' AS Action FROM client ORDER BY PRENOM_CL ASC";
            break;
        case 2: // Sort by Age (Ascending)
            queryStr = "SELECT CIN, NOM_CL, PRENOM_CL, TEL_CL, ADRESSE_CL, GENDER, AGE, EMAIL, 'Update' AS Action, 'Delete' AS Action FROM client ORDER BY AGE ASC";
            break;
        case 3: // Sort by CIN (Ascending)
            queryStr = "SELECT CIN, NOM_CL, PRENOM_CL, TEL_CL, ADRESSE_CL, GENDER, AGE, EMAIL, 'Update' AS Action, 'Delete' AS Action FROM client ORDER BY CIN ASC";
            break;
        default:
            queryStr = "SELECT CIN, NOM_CL, PRENOM_CL, TEL_CL, ADRESSE_CL, GENDER, AGE, EMAIL, 'Update' AS Action, 'Delete' AS Action FROM client";
            break;
    }

    clientModel->setQuery(queryStr);

    clientModel->setHeaderData(0, Qt::Horizontal, QObject::tr("CIN"));
